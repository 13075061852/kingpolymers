...
def run():
 with tempfile.TemporaryDirectory() as directory:
  ...
  for machine,spec in specs.items():
   ...
   program="const M=require('./static/barrel-models.js');const x=JSON.parse(require('fs').readFileSync(0,'utf8'));console.log(JSON.stringify({rows:M.rows(x.machine,x.spec,x.ports),zones:M.zones(x.machine,x.spec,x.ports),warnings:M.warnings(x.machine,x.spec,x.ports)}))"
   result=json.loads(subprocess.check_output(['node','-e',program],input=json.dumps(dict(machine=machine,spec=spec,ports=ports)).encode(),cwd=server.ROOT))
