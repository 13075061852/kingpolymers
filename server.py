...
STATIC = ROOT / "static"
DATA = Path(os.environ.get("GJ_DATA_DIR", str(ROOT / "data"))).resolve()
BACKUPS = Path(os.environ.get("GJ_BACKUP_DIR", str(ROOT / "backups"))).resolve()
DB_PATH = DATA / "guangjun_screw.db"
SEED_PATH = ROOT / "data" / "seed.json"
ASSET_MANIFEST = STATIC / "assets" / "manifest.json"
        ...
        try:
            ...
            if p=="/api/bootstrap":
                with db() as c:
                    ...
                    assets=loads(ASSET_MANIFEST.read_text(encoding="utf-8"),{}) if ASSET_MANIFEST.exists() else {}
                    self.send_json({"machines":machine_specs(),"components":component_rows(c),"templates":template_rows(c),"projects":project_rows(c,False),"settings":settings,"assets":assets,"auth":self.auth_context()}); return
