...
function editableDrawingSVG(exportMode=false,compact=false){
 ...
 let labelCum=0;const labelXs=state.sequence.map(name=>{const length=comp(name).length,span=DrawingAppearance.visibleSpan(labelCum-s.entry_offset,length),x=right-(s.entry_offset+(span.start+span.end)/2)*scale;labelCum+=length;return x}),order=labelXs.map((_,i)=>i).sort((a,b)=>labelXs[a]-labelXs[b]),gap=Math.min(labelFont+1.5,1120/Math.max(1,order.length-1));
