...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.ComponentModels=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
 ...
 const colors={GFA:'#111111',GFF:'#ad7d0d',KB:'#173ad4',KBX:'#173ad4',GFM:'#147c39',SME:'#147c39',KS:'#6642a0',Spacer:'#596973'};
 function parse(name){
 ...
 function symbol(c,spec={},options={}){
  const p=model(c),D=Number(spec.diameter)||60,L=p.length||60,ink=typeof options.color==='string'&&/^#[0-9a-f]{6}$/i.test(options.color)?options.color:(colors[p.type]||'#888'),id='model-'+String(options.id||p.id||p.label).replace(/[^a-zA-Z0-9_-]/g,'_');
  ...
  let art='',detailDefs='';const line=D*(options.thumbnail?.04:.029);
  if(p.type==='KB'||p.type==='KBX'){
   ...
   const period=p.pitch/p.lobes,depth=D*.11,top=D*.035,bottom=D*.965,shift=period,thickness=Math.min(period*.105,D*.027);
   ...
   const spans=[];
   for(let x=0;x<L-1e-7;x+=period){const end=Math.min(L,x+period);spans.push({x,end,depth:depth*((end-x)/period)**2})}
   let a=`M0 ${n(top)}`;
   for(const s of spans)a+=` Q${n((s.x+s.end)/2)} ${n(top+2*s.depth)} ${n(s.end)} ${n(top)}`;
   a+=`L${n(L)} ${n(bottom)}`;
   for(const s of [...spans].reverse())a+=` Q${n((s.x+s.end)/2)} ${n(bottom-2*s.depth)} ${n(s.x)} ${n(bottom)}`;
   a+='Z';
   ...
   for(const x of [0,L])art+=`<path data-end-face="${x}" d="M${x} ${n(top)}V${n(bottom)}" fill="none" stroke="${ink}" stroke-width="${n(line)}"/>`;
