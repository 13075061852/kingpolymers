...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.ComponentModels=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
 ...
 function symbol(c,spec={},options={}){
  ...
  if(p.type==='KB'||p.type==='KBX'){
   ...
   const period=p.pitch/p.lobes,top=D*.035,bottom=D*.965,shift=period*.66,thickness=Math.min(period*.105,D*.027);
   if(!(period>0)||L/period>500)return '';
   // The installed block is coaxial: straight OD envelope and square end faces.
   // Only the internal flight lines are inclined; do not skew the whole block.
   art+=`<path data-block-envelope="axial" d="M0 ${n(top)}H${n(L)}V${n(bottom)}H0Z" fill="white" stroke="${ink}" stroke-width="${n(line)}"/>`;
   // Clip flight strokes, including their joins, to the straight block envelope.
   // The outer x-only clip cannot stop acute stroke miters protruding above/below it.
   detailDefs=`<clipPath id="${id}-flights"><rect x="0" y="${n(top)}" width="${n(L)}" height="${n(bottom-top)}"/></clipPath>`;
   art+=`<g data-flight-bounds="axial" clip-path="url(#${id}-flights)" stroke-linejoin="bevel" stroke-linecap="butt">`;
   ...
   art+='</g>';
   art+=`<path data-joint-outline="flush" d="M0 ${n(top)}H${n(L)}V${n(bottom)}H0Z" fill="none" stroke="${ink}" stroke-width="${n(line)}" stroke-linejoin="miter"/>`;
   // Draw both vertical end faces last so each physical block stays distinct.
