...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.ComponentModels=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
 ...
 function symbol(c,spec={},options={}){
  ...
  let art='',detailDefs='';const installed=options.presentation==='installed';const line=D*(options.thumbnail?.04:.029);
  if(p.type==='KB'||p.type==='KBX'){
   ...
   // Catalog retains the original contour; installed presentation has flush end faces.
   const scallop=x=>{const t=((x%period)+period)%period/period;return 4*depth*t*(1-t)};
   for(const x of [0,L])art+=`<path data-end-face="${x}" d="M${x} ${n(installed?top:top+scallop(x))}V${n(installed?bottom:bottom-scallop(x-shift))}" fill="none" stroke="${ink}" stroke-width="${n(line)}"/>`;
