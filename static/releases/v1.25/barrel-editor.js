...
function editableDrawingSVG(exportMode=false,compact=false){
 ...
 const showModels=!exportMode||!compact,labelY=bandY+bandH+38,labelFont=8.5;
 ...
 const H=Math.max(exportMode&&!compact?485:330,showModels&&state.sequence.length?Math.ceil(labelY+longest+55):0);
 const bx=mm=>right-(mm+s.entry_offset)*scale,fit=BarrelVisuals.axialFit(s,rows,v.total,v.target),entry=rows[0]?.role==='feed'?BarrelVisuals.inlet(bx(0),s.entry_offset*scale,bandY,bandH,s.entry_offset):'';let barrel='',elements='',labels='';
 ...
 let labelCum=0;const labelXs=state.sequence.map(name=>{const length=comp(name).length,x=right-(labelCum+length/2)*scale;labelCum+=length;return x}),order=labelXs.map((_,i)=>i).sort((a,b)=>labelXs[a]-labelXs[b]),gap=Math.min(labelFont+1.5,1120/Math.max(1,order.length-1));
 let rail=20;for(const i of order){labelXs[i]=Math.max(labelXs[i],rail);rail=labelXs[i]+gap}
 rail=1180;for(const i of [...order].reverse()){labelXs[i]=Math.min(labelXs[i],rail);rail=labelXs[i]-gap}
 let cum=0;for(const [i,name] of state.sequence.entries()){
  const c=comp(name),w=c.length*scale,x=right-(cum+c.length)*scale;elements+=exactElementImage(x,w,c,i,bad.has(i),bandY,bandH,!exportMode);
  if(showModels){const cx=x+w/2,lx=labelXs[i];labels+=`<g class="installed-model-label" data-model-label-index="${i}" pointer-events="none"><path d="M${cx} ${bandY+bandH+19}L${lx} ${labelY-4}" fill="none" stroke="#68747b" stroke-width=".65"/><text x="${lx}" y="${labelY}" transform="rotate(-90 ${lx} ${labelY})" text-anchor="end" font-family="Courier New,Microsoft YaHei,monospace" font-size="${labelFont}" fill="${bad.has(i)?'#c22':'#222'}">${esc(name)}</text></g>`}cum+=c.length;
 }
