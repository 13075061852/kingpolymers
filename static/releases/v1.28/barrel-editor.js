...
function editableDrawingSVG(exportMode=false,compact=false){
 ...
 const showModels=!exportMode||!compact,labelY=bandY+bandH+40,labelFont=10.5,labelGap=17,labelLanes=Math.max(1,Math.ceil(state.sequence.length/66));
 // Reserve real space for full vertical model names without moving the screw.
 const longest=Math.max(0,...state.sequence.map(name=>[...name].reduce((sum,c)=>sum+(c.codePointAt(0)>255?1.2:.82),0)*labelFont));
