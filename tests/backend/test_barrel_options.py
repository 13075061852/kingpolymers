...
def run():
 with tempfile.TemporaryDirectory() as temp:
  ...
  for machine,spec in server.machine_specs().items():
   def parity(ports):
    code="const M=(await import('./frontend/src/domain/barrel-models.js')).default,x=JSON.parse((await import('node:fs')).readFileSync(0));console.log(JSON.stringify({layout:M.normalize(x.machine,x.spec,x.ports.barrel_layout),summary:M.summary(x.machine,x.spec,x.ports),zones:M.zones(x.machine,x.spec,x.ports),warnings:M.warnings(x.machine,x.spec,x.ports),target:M.targetLength(x.machine,x.spec,x.ports)}))"
    js=json.loads(subprocess.check_output(['node','--input-type=module','-e',code],input=json.dumps(dict(machine=machine,spec=spec,ports=ports)).encode(),cwd=server.ROOT))
